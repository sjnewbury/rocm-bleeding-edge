...
PATCHES=(
	 ...
	 "${FILESDIR}"/${PN}-5.7.1-msgpack-c.patch
	 "${FILESDIR}"/${PN}-5.7.1-fix-fallback-arch-build.patch
	 "${FILESDIR}"/${PN}-6.0.0-gfx902.patch
)
...
src_prepare() {
	...
	sed -e "s,\@LLVM_PATH\@,$(get_llvm_prefix ${LLVM_MAX_SLOT}),g" "${FILESDIR}"/${PN}-5.7.1-gentoopath.patch > "${S}"/gentoopath.patch || die
	...
	sed -e "s,\${Tensile_ROOT}/bin/,,g" -i cmake/TensileConfig.cmake || die # ${Tensile_ROOT}/bin does not exists; call command directly
...
src_install() {
	...
	doins -r Configs Perf ReplacementKernels* Source CustomKernels
