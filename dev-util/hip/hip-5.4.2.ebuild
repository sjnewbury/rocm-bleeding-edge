...
inherit cmake docs llvm prefix python-any-r1 flag-o-matic
...
SRC_URI="https://github.com/ROCm-Developer-Tools/hipamd/archive/rocm-${PV}.tar.gz -> rocm-hipamd-${PV}.tar.gz
	...
	https://github.com/ROCm-Developer-Tools/HIPIFY/archive/rocm-${PV}.tar.gz -> hipify-clang-${PV}.tar.gz
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-5.3.0-correct-sample-install-location.patch"
	"${FILESDIR}/${PN}-5.3.0-remove-cmake-doxygen-commands.patch"
)
...
src_prepare() {
	...
	sed -e "/CMAKE_CXX_COMPILER/d" \
		-e "/CMAKE_CXX_COMPILER/d" \
		-e "/find_package(LLVM REQUIRED CONFIG PATHS /s:\${CMAKE_PREFIX_PATH}:$(get_llvm_prefix ${LLVM_MAX_SLOT}):" \
		-i ${HIC_S}/CMakeLists.txt || die

	pushd ${HIC_S} || die
	eapply "${FILESDIR}/${PN}-5.4.0-hipify-clang-llvm-link.patch"
	eapply "${FILESDIR}/${PN}-5.3.0-hipify-clang-install.patch"
	eapply "${FILESDIR}/${PN}-5.3.0-hipify-clang-sys-include.patch"
	eapply "${FILESDIR}/${PN}-5.4.0-hipify-clang-llvm16.patch"
	sed -e "s,@CLANG_INCLUDE_PATH@,${CLANG_RESOURCE_DIR}/include," \
		-i ${HIC_S}/src/main.cpp || die
	popd

	pushd ${HIP_S} || die
	eapply "${FILESDIR}/stable/new-dir-layout.patch"
	eapply "${FILESDIR}/stable/no-rel-clang-path.patch"
	eapply "${FILESDIR}/${PN}-5.1.3-clang-include-path.patch"
	eapply "${FILESDIR}/${PN}-5.1.3-rocm-path.patch"
	eapply "${FILESDIR}/${PN}-5.4.0-correct-ldflag.patch"
...
src_install() {
	...
	rm "${ED}/usr/$(get_libdir)/.hipInfo" "${ED}/usr/bin/.hipVersion" || die
}
